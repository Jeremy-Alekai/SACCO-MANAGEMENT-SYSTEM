...
Public Class Floans_list
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles btnAdd_loan.Click
        ...
        Try
            Dim str As String = "INSERT INTO loans(MemberID,Name,Employment_Postion,Payroll_num,Loan_Amount,Loan_interest,Monthly_NetIncome,My_Tel,email,Emp_name,Employer_tel,Emp_address,Terms_of_service,expiry_date,Position_in_sacco) VALUES('" & txtMemberID.Text & "','" & txtName.Text & "','" & txtEmp_position.Text & "','" & txtPayroll_num.Text & "','" & txtLoanAmount.Text & "','" & txtIoan_Interest.Text & "','" & txtExpenditure.Text & "','" & txtMy_Tel.Text & "','" & txtEmail.Text & "','" & txtEmp_nam.Text & "','" & txtEmp_Tel.Text & "','" & txtEmp_Address.Text & "','" & cboTerms_of_service.Text & "','" & txtExpiry.Text & "','" & cboSACCO_Position.Text & "')"
            ourconn.Open()
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles btnEdit.Click
        ...
        Dim LoanAmount_StrBind As String
        ...
        LoanAmount_StrBind = CType(DtaView.Item("Loan_Amount").ToString, String)
        ...
        txtLoanAmount.Text = LoanAmount_StrBind
        txtEmail.Text = email_StrBind

        txtEmp_nam.Text = EmpName_StrBind
        txtEmp_Tel.Text = EmployerTel_StrBind
        ...
        txtExpenditure.Text = MonthlyNetIncome_StrBind
    ...
    Private Sub TextBox16_TextChanged(sender As Object, e As EventArgs) Handles txtLoanAmount.TextChanged
        'txtTOTLoan_Amount.Text = (CInt(txtLoanAmount.Text) * (CInt(txtIoan_Interest.Text) / 100)).ToString
    End Sub
    ...
    Private Sub btnSave_Changes_Click(sender As Object, e As EventArgs) Handles btnSave_Changes.Click
        Try
            ourconn.Close()
            ourconn.Open()
            Dim cmd = New MySqlCommand("UPDATE Loans SET MemberID=@OurMemberID,Name=@OurName,Employment_Postion=@OurEmployment_Postion,Payroll_num=@OurPayroll_num,Loan_Amount=@OurLoan_Amount,Loan_interest=@OurLoan_interest,Monthly_NetIncome=@OurMonthly_NetIncome,My_Tel=@OurMy_Tel,email=@Ouremail,Emp_name=@OurEmp_name,Employer_tel=@OurEmployer_tel,Emp_address=@OurEmp_address,Terms_of_service=@OurTerms_of_service,expiry_date=@Ourexpiry_date,Position_in_sacco=@OurPosition_in_sacco where Loan_No='" & LoanID.Text.Trim & "'", ourconn)
            ...
            With cmd
                ...
                .Parameters.AddWithValue("OurEmployment_Postion", txtEmp_position.Text.Trim)
                .Parameters.AddWithValue("OurPayroll_num", txtPayroll_num.Text.Trim)
                .Parameters.AddWithValue("OurLoan_Amount", txtLoanAmount.Text.Trim)
                .Parameters.AddWithValue("OurLoan_interest", txtIoan_Interest.Text.Trim)
                .Parameters.AddWithValue("OurMonthly_NetIncome", txtExpenditure.Text.Trim)
                .Parameters.AddWithValue("OurMy_Tel", txtMy_Tel.Text.Trim)
                .Parameters.AddWithValue("Ouremail", txtEmail.Text.Trim)
                .Parameters.AddWithValue("OurEmp_name", txtEmp_nam.Text.Trim)
                .Parameters.AddWithValue("OurEmployer_tel", txtEmp_Address.Text.Trim)
                .Parameters.AddWithValue("OurEmp_address", txtEmp_Address.Text.Trim)
                .Parameters.AddWithValue("OurTerms_of_service", cboTerms_of_service.Text.Trim)
                .Parameters.AddWithValue("Ourexpiry_date", txtExpiry.Text.Trim)
                .Parameters.AddWithValue("OurPosition_in_sacco", txtName.Text.Trim)
            End With
    ...
    Private Sub txtTOT_TextChanged(sender As Object, e As EventArgs)
        ''txtTOT.Text = CInt(txtIoan_Interest.Text / 100) * CInt(txtNet_Income.Text)
    End Sub
End Class